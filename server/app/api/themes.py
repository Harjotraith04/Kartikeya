...
from app.schemas.theme import ThemeOut, ThemeCreate, ThemeUpdate, ThemeWithCodes, AttachCodes
...
@router.put("/attach-codes", response_model=List[CodeOut])
def assign_codes_to_theme(
    assignment: AttachCodes,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
