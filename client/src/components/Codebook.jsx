...
function Codebook({ codeAssignments, projectId, codes, setCodes, onCodesUpdated, documents = [], codebooks = [] }) {
  ...
  const [selectedCodesForGrouping, setSelectedCodesForGrouping] = useState([]); // For sub-grouping / theming
  ...
  // Function to assign selected codes to chosen theme
  const handleAssignThemeSubmit = async () => {
    ...
    try {
      ...
      if (!themeId) {
        alert('Please select or create a theme.');
        return;
      }
      await codesApi.assignCodesToTheme(selectedCodesForGrouping.map(id=>parseInt(id)), themeId);
      alert('Codes successfully assigned to theme.');
  ...
  const handleSubGroupSubmit = async () => {
    ...
    try {
      const updatedCodes = await codesApi.assignGroupToCodes(selectedCodesForGrouping.map(id=>parseInt(id)), newSubGroupName.trim());
  ...
  const handleThemes = async () => {
    if (selectedCodesForGrouping.length === 0) {
      alert('Please select at least one code to assign to a theme.');
      return;
    }
    try {
      // Load existing themes for this project
      const projectThemes = await themesApi.getProjectThemes(projectId);
    ...
    <Box sx={{ 
      ...
      <Box sx={{ display: 'flex', flexGrow: 1, overflow: 'hidden' }}>
        ...
        <Box sx={{ 
          ...
          <FrostedGlassPaper sx={{ p: 2, mb: 2, flexShrink: 0 }}>
          ...
          <Box sx={{ 
            ...
            minHeight: 0, 
            p: isMobile ? 0 : 1,
            maxHeight: 'calc(100vh - 180px)', // Ensures scroll bar appears when needed
            '&::-webkit-scrollbar': {
              width: '8px',
            },
      ...
      <Dialog open={themesDialogOpen} onClose={() => setThemesDialogOpen(false)}>
        <DialogTitle sx={{ fontWeight: 600 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <PaletteIcon color="secondary" /> Assign to Theme
          </Box>
        </DialogTitle>
        <DialogContent sx={{ pt: 3 }}>
          <Typography variant="body2" sx={{ mb: 2 }}>
            Assign {selectedCodesForGrouping.length} selected code{selectedCodesForGrouping.length!==1?'s':''} to a theme.
          </Typography>
          <FormControl component="fieldset" sx={{ mb: 2 }}>
            ...
            >
              {themes.map(theme=> (
                <FormControlLabel key={theme.id} value={theme.id} control={<Radio />} label={theme.name} />
              ))}
              {themes.length===0 && <Typography variant="caption">No themes yet.</Typography>}
            </RadioGroup>
          </FormControl>
          <TextField
            fullWidth
            label="Or create new theme"
            value={newThemeName}
        ...
        <DialogActions sx={{ p: 3, pt:1 }}>
          <Button onClick={() => setThemesDialogOpen(false)} color="inherit">Cancel</Button>
          <Button variant="contained" startIcon={<PaletteIcon />} onClick={handleAssignThemeSubmit}>Assign Theme</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
