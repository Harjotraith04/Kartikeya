...
const MergingPage = ({ projectId, codes = [], codeAssignments = [], documents = [], codebooks = [], refreshProjectData }) => {
  ...
  const handleCodeDelete = (assignmentId, codeType) => {
    ...
    setDeletedCodes(prev => {
      ...
      const isDeleting = !newSet.has(key);
      ...
      // Update assignment status based on deletion state
      if (isDeleting) {
        // Check if both manual and AI codes will be deleted
        const manualKey = `${assignmentId}_manual`;
        const aiKey = `${assignmentId}_ai`;
        const willBothBeDeleted = (codeType === 'manual' ? true : newSet.has(manualKey)) && 
                                 (codeType === 'ai' ? true : newSet.has(aiKey));
        ...
        if (willBothBeDeleted) {
          ...
          saveAssignmentStatus(assignmentId, 'rejected');
        }
      } else {
        // Restoring a code - set back to pending if it was rejected
        const currentStatus = assignmentStatus[assignmentId];
        if (currentStatus === 'rejected') {
          ...
          // Save status to backend
          saveAssignmentStatus(assignmentId, 'pending');
        }
      }
  ...
  // Filter for Compare tab - only show accepted assignments
  ...
  // Filter for Status tab - categorize by status (only pending and accepted)
      ...
      {activeTab === 3 && (
        <Box sx={{ flexGrow: 1, overflow: 'auto', maxHeight: 'calc(100vh - 280px)' }}>
          {['pending', 'accepted'].map((status) => {
            const statusData = sortedData[status] || [];
            const statusColor = {
                        ...
                        <TableBody>
                          {statusData.map((item) => (
                            <TableRow key={item.id} hover sx={{
                              ...
                              <TableCell>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  {getCodeData(item).manual.name && (
                                    <AnimatedChip
                                      label={getCodeData(item).manual.name}
                                      sx={{
                                        backgroundColor: getCodeData(item).manual.deleted 
                                          ? alpha(theme.palette.error.main, 0.1)
                                          : getCodeData(item).manual.color,
                                        color: getCodeData(item).manual.deleted 
                                          ? theme.palette.error.main
                                          : theme.palette.getContrastText(getCodeData(item).manual.color),
                                        '& .MuiChip-label': { fontWeight: 500 },
                                        textDecoration: getCodeData(item).manual.deleted ? 'line-through' : 'none'
                                      }}
                                    />
                                  )}
                                </Box>
                              ...
                              <TableCell>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  {getCodeData(item).ai.name && (
                                    <AnimatedChip
                                      label={getCodeData(item).ai.name}
                                      sx={{
                                        backgroundColor: getCodeData(item).ai.deleted 
                                          ? alpha(theme.palette.error.main, 0.1)
                                          : alpha(getCodeData(item).ai.color, 0.8),
                                        color: getCodeData(item).ai.deleted 
                                          ? theme.palette.error.main
                                          : theme.palette.getContrastText(getCodeData(item).ai.color),
                                        '& .MuiChip-label': { fontWeight: 500 },
                                        textDecoration: getCodeData(item).ai.deleted ? 'line-through' : 'none'
                                      }}
                                    />
                                  )}
                                </Box>
                                    ...
                                    <Tooltip title="Accept assignment">
                                      <GradientButton
                                        ...
                                        onClick={() => {
                                          ...
                                          // Save status to backend
                                          saveAssignmentStatus(item.id, 'accepted');
                                        }}
                                      >
                                        Accept
                                      </GradientButton>
                                    ...
                                    <Tooltip title="Mark as pending">
                                      <GradientButton
                                        ...
                                        onClick={() => {
                                          ...
                                          // Save status to backend
                                          saveAssignmentStatus(item.id, 'pending');
                                        }}
                                      >
                                        Mark Pending
                                      </GradientButton>
